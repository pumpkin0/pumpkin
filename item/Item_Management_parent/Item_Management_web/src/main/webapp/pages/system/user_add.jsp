...
<html>
	<head>
		...
		<script type="text/javascript">
			$(function(){
				//查询所有的权限数据
				$.post("${pageContext.request.contextPath}/roleAction_findAll.action",null,function(data){
					console.info(data);
					//将数组中权限数据遍历展示在页面中
					for(var i=0;i<data.length;i++){
						var id = data[i].id;
						var name = data[i].name;
						$("#roleTD").append('<input type="checkbox" name="roleIds" value="'+id+'" /><label for='+id+'>' + name+' </label>');
					}
				})
	...
	<body class="easyui-layout" style="visibility:hidden;">
		...
		<div region="center" style="overflow:auto;padding:5px;" border="false">
			<form id="userForm" method="post" action="${pageContext.request.contextPath}/userAction_save.action">
