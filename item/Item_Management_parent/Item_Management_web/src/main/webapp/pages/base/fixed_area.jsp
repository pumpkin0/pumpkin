...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function doAssociations(){
				...
				if(rows.length == 1){
					$.ajax({ 
						url: "${pageContext.request.contextPath}/customer_findNotAssociation.action",  
						success: function(data){
							for(item in data){
								$('#noassociationSelect').append('<option value='+data[item].id+'>'+data[item].info+'</option>');
							}
					}});
					
					$.ajax({ 
						url: "${pageContext.request.contextPath}/customer_findAssociation.action?fixedAreaId="+rows[0].id,  
						success: function(data){
							for(item in data){
								$('#associationSelect').append('<option value='+data[item].id+'>'+data[item].info+'</option>');
							} 
					}});
					...
					$.messager.alert('tip','同时只能修改一条数据!！','warning');
				}
			}
			...
			var toolbar = [ {
				...
				handler : function(){
					...
					var rows = $("#grid").datagrid('getSelections');
					if(rows.length==1){
						//使用的js方式，动态返回符合条件的快递员的信息
						$('#courierId').combobox({    
						    url:'${pageContext.request.contextPath}/fixedArea_findCourierList.action?id='+rows[0].id,    
	...
	<body class="easyui-layout" style="visibility:hidden;">
			...
			<div style="overflow:auto;padding:5px;" border="false">
				<form id="fixedAreaForm" method="post" action="${pageContext.request.contextPath}/fixedArea_save.action">
					<table class="table-edit" width="80%" align="center">
						...
						<tr height="300">
						<tdvalign="top">关联分区</td>
						<td>
							<table id="subareaGrid"  class="easyui-datagrid" border="false" style="width:300px;height:300px" 
								data-options="url:'${pageContext.request.contextPath}/subarea_findMeetNull.action',fitColumns:true,singleSelect:false">
								<thead>
									<tr>
		...
		<!-- 关联客户窗口 -->
		<div modal="true" class="easyui-window" title="关联客户窗口" id="customerWindow" collapsible="false" closed="true" minimizable="false" maximizable="false" style="top:20px;left:200px;width: 800px;height: 300px;">
			<div style="overflow:auto;padding:5px;" border="false">
				<form id="customerForm" action="${pageContext.request.contextPath}/fixedArea_assignCustomers2FixedArea.action" method="post">
					<table class="table-edit" width="80%" align="center">
						<tr class="title">
					...
					<table class="table-edit" width="80%" align="center">
						...
						<tr>
							...
							<td>
								...
								<input data-options="ditable:false"
									 type="text" name="courierId" class="easyui-combobox" required="true" id="courierId"/>
							</td>
						</tr>
						<tr>
							<td>选择收派时间</td>
							<td>
								<select required="true" class="easyui-combobox" name="takeTimeId" id="takeTimeId">
							...
							<td colspan="3">
								...
								<script type="text/javascript">
									$('#associationCourierBtn').click(function(){
										...
										var takeTimeValue = $('#takeTimeId').combo('getValue');	
										if(takeTimeValue == -1){
											$.messager.alert("警告","必须确定接派时间","warning");
										}else {
											$('#courierForm').submit();
										}
									})
