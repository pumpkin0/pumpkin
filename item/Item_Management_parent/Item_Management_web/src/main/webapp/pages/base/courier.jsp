	...
	<head>
		...
		<script type="text/javascript">
			...
			function doDelete(){
				...
				if(rows.length < 1){
					$.messager.alert('tip','选中的条数必须大于1条！','info');
				}else{
					    ...
					    if (r){    
					       var array = new Array();
   							for(var i = 0; i < rows.length; i++){
   								...
   								var ids =array.join(",");
   								//这儿可以直接对页面数据进行刷新
   								//方法一：整个页面location.href
   								//window.location.href="${pageContext.request.contextPath}/courier_delete.action?ids="+ids;
   								//方式二：利用ajax请求
   								$.ajax({
   								  url: "${pageContext.request.contextPath}/courier_delete.action?ids="+ids,
			...
			var toolbar = [ {
				...
				handler : function(){
					var rows = $('#grid').datagrid('getSelections'); 
					if(rows.length != 1){
						$.messager.alert('tip','选中的条数不能大于1！','info');
					}else{
						//数据的回显
						//console.info(rows[0]);
						$('#addWindow').form('load',rows[0]);
						$('#standardname').combobox('setValue', rows[0].standard.name);
						$('#addWindow').window('open');
					}
		...
		<div class="easyui-window" title="对收派员进行添加或者修改" id="addWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
			<div region="north" style="height:31px;overflow:hidden;" split="false" border="false">
				<div class="datagrid-toolbar">
					...
					<script type="text/javascript">
						$("#save").click(function(){
							var r = $("#newCourier").form('validate');
							if(r){
								$("#newCourier").submit();
							}
			...
			<div region="center" style="overflow:auto;padding:5px;" border="false">
				<form id="newCourier" method="post" action="${pageContext.request.contextPath }/courier_save.action">
						...
						<tr>
							...
							<script type="text/javascript">
								/* $('#searchBtn').click(function(){
									 $('#searchForm').form('submit', {    
									    ...
									    onSubmit: function(param){    
									        param.page = $('#grid').datagrid('getPager').data("pagination").options.pageNumber;    
									        param.rows = $('#grid').datagrid('getPager').data("pagination").options.pageSize;    
									    }    
									}); 
									$('#searchWindow').window('close');  
									window.location.href='${pageContext.request.contextPath }/pages/base/courier.jsp';
								...
								$("#searchBtn").click(function(){
										...
										//2、提交url请求，提交参数：1、分页需要page,rows 2、查询条件值
										$('#grid').datagrid('load',  condition);  
										//将窗口关闭
										$("#searchWindow").window("close")
									})
									
									//将表单序中输入项序列化为json格式-json对象  {"输入项name":"输入项的值"}
